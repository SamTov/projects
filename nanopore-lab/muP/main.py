...
# Data loading code
from data_prep import TrainingDataset, ValidationDataset
...
def coord_check(
    ...
    trainset = TrainingDataset(
            dataset_dir="/work/jhossbach/Image_Dataset_old/dataset_all", transform=transform_train
    )
...
def train(epoch, net):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        outputs = net(inputs)
        loss = criterion(outputs, targets) / batch_accumulation  # Normalize the loss

        loss.backward()  # Accumulate gradients
        train_loss += loss.item() * batch_accumulation  # Adjusting for the normalization

        _, predicted = outputs.max(1)
        total += targets.size(0)
        correct += predicted.eq(targets).sum().item()

        # Perform optimization step after 'batch_accumulation' iterations
        if (batch_idx + 1) % batch_accumulation == 0 or (batch_idx + 1) == len(trainloader):
            optimizer.step()  # Update weights
            optimizer.zero_grad()  # Reset gradients for the next set of batches
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seed', type=int, default=1111,
                        help='random seed')
    parser.add_argument('--accumulate_batches', type=int, default=500,
                        help='batch accumulation parameter')
    ...
    if not args.save_base_shapes:
        ...
        trainset = TrainingDataset(
            dataset_dir="/work/jhossbach/Image_Dataset_old/dataset_all", transform=transform
        )
        ...
        testset =  ValidationDataset(
            dataset_dir="/work/jhossbach/Image_Dataset_old/dataset_all") #, transform=transform_test
        # )
        testloader = torch.utils.data.DataLoader(
            testset, batch_size=args.test_batch_size, shuffle=False, num_workers=args.test_num_workers
            )

        classes = np.linspace(0, 41, 42, dtype=int)

        batch_accumulation = args.accumulate_batches
    ...
    if args.coord_check:
        ...
        coord_check(
            ...
            arch=args.arch, 
            base_shapes=args.load_base_shapes,
