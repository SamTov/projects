...
def train(
  ...
  for step in range(start_step, loop_steps):
    ...
    trajectories, rewards = process_experience(
        all_experiences,
        config.actor_steps,
        config.num_agents,
        config.gamma,
    ...
    np.save(f"/work/stovey/novely-model-study/ppo/rewards_{step}.npy", rewards)
    ...
    # Save model state
    checks = {'model': state}
    orbax_checkpointer.save(f"/work/stovey/novely-model-study/ppo/model_{step}", checks)
