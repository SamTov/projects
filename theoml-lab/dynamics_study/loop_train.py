...
def train_model(data, size, epochs, lr, batch_size, architecture, width, depth, activation, ntk_batch, input_shape, accuracy, loss_fn):

    ds_switch = {
        "fuel": nl.data.MPGDataGenerator,
    }

    # generator = ds_switch[args.data](args.size)
    # size = args.size
    # epochs = args.epochs
    # lr = args.lr
    ...
    loss_fn_switch = {
        "mean_power_2": nl.loss_functions.MeanPowerLoss(order=2),
        "mean_power_4": nl.loss_functions.MeanPowerLoss(order=4),
        "cross_entropy": nl.loss_functions.CrossEntropyLoss()
    }
    loss_fn = loss_fn_switch[loss_fn]
