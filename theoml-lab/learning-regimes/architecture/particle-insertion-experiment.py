...
class Generator(nl.data.DataGenerator):
    def __init__(self, n_samples: int, discriminator: callable = linear_boundary):
        ...
        class_one_indices = np.where(train_targets == 0)[0][:n_samples]
        class_two_indices = np.where(train_targets == 1)[0][:n_samples]
      
        indices = np.hstack((class_one_indices, class_two_indices))

        self.train_ds = {
            "inputs": np.take(train_data, indices, axis=0),
            "targets": np.take(train_targets, indices, axis=0)
        }
...
def main(
    ...
    onp.save(f"{name}.npy", experiment_results)
...
main(ds_size=7, batch_size=14, epochs=1000000, name="long-time.npy")
